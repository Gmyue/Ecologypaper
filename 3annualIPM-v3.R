...
if (length(excel_sheets(file_24)) >= 3) {
  sheet3_24 <- read_sheet(file_24, 3)
} else {
  sheet3_24 <- NULL
}
...
if (!all(c("Drought","Fertilization","HM") %in% names(end_24))) {
  if (is.null(sheet3_24)) {
    stop("end_24 缺处理列，且 Sheet3 不存在，无法补")
  }
  treat_24 <- sheet3_24 %>%
...
# 检查响应尺度，确认先验需求
cat("\n---- 响应尺度诊断 ----\n")
cat("log(N_end + 1) 均值:", mean(log(end_dat$N_end + 1)), "\n")
cat("log(N0_next_int + 1) 均值:", mean(log(rec_dat$N0_next_int + 1)), "\n")
cat("Height 均值:", mean(h_dat$Height, na.rm = TRUE), "\n")

# =========================================================
# 7. 拟合模型（放宽先验）
# =========================================================
# 7.1 年末数量
#     N_end 均值约 100 → log(100) ≈ 4.6，所以截距先验用 normal(0, 5)
m_end <- brm(
...
# 7.2 年末株高
#     Height 均值 ~ 150，截距先验用 normal(150, 50)
m_h <- brm(
...
# 7.3 补充模型（关键修复）
#     N0_next 均值 ~ 500 → log(500) ≈ 6.2，截距先验用 normal(0, 5)
m_rec <- brm(
...
treat_summary <- end_dat %>%
  ...
  summarise(
    ...
    p_surv      = mean(N_end / pmax(N0, 1), na.rm = TRUE),
    .groups = "drop"
  )
...
lambda_res <- treat_combos %>%
  left_join(treat_summary, by = c("Drought","Fertilization","HM")) %>%
  rowwise() %>%
  mutate(
    N0_next_pred = {
      nd <- data.frame(
        ...
        HM            = HM,
        log_N_end_p1  = log(N_end_mean + 1),
        mean_H        = mean_H_mean,
        Block         = end_dat$Block[1]
      )
      mean(posterior_epred(m_rec, newdata = nd, re_formula = NA))
    },
    lambda = N0_next_pred / N0_mean
  ) %>%
  ungroup()
...
build_IPM_one <- function(post_h, post_rec,
  ...
  p_k <- p_k / sum(p_k)
  G <- matrix(rep(p_k, each = 5), nrow = 5, byrow = TRUE) * p_surv
...
A_demo <- build_IPM_one(
  ...
  N_end  = mean(treat_summary$N_end_mean),
  mean_H = mean(treat_summary$mean_H_mean),
  p_surv = mean(treat_summary$p_surv)
)
colSums(A_demo)
